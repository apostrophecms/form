    ...
    collectToSkip(input, conditionals, skipFields) {
      for (const name in conditionals) {
        for (let value in conditionals[name]) {
          ...
          // Strip surrounding quotes from condition values (handles common user entry mistakes)
          if (typeof conditionValue === 'string') {
            conditionValue = conditionValue.replace(/^["']|["']$/g, '');
          }

          // Normalize boolean representations to handle widget type mismatches
          if (inputValue === 'on' || inputValue === 'true') {
            inputValue = true;
          }
          if (inputValue === 'off' || inputValue === 'false') {
            inputValue = false;
          }

          if (conditionValue === 'on' || conditionValue === 'true') {
            conditionValue = true;
          }
          if (conditionValue === 'off' || conditionValue === 'false') {
            conditionValue = false;
          }

          if (inputValue !== conditionValue) {
            conditionals[name][conditionValue].forEach(field => {
